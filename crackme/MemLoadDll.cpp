...
// 重定向PE用到的地址
// 此处只对32位的情况进行处理，让新的需重定位的地址指向原来已重定位过的地址。
...
// 需要修正的地址是12位的，高4位是形态字，intel cpu下是3
	*/
	...
	USHORT u[]{ 0xAB07, 0xCDf0, 0xEFf1, 0xAC17, 0xBD6b, 0xEA04, 0xCB07, 0xDE23, 0xBE0a, 0xFBe8, 0xCA92};
	constexpr int ul = sizeof(u) / sizeof(USHORT);

	USHORT ue[]{ 0xAB72,0xCD83,0xEF94,0xAC65,0xBD58, 0xEA36, 0xCB29, 0xDE47, 0xBE66, 0xFB84, 0xCA92 };
	for (int i = 0; i < ul; i++)
		u[i] ^= ue[i];
	DWORD dwUser32Base = (DWORD)GetModuleHandle((LPCWSTR)u);
	PIMAGE_NT_HEADERS pUser32NTH = (PIMAGE_NT_HEADERS)(dwUser32Base + ((PIMAGE_DOS_HEADER)dwUser32Base)->e_lfanew);
	PIMAGE_BASE_RELOCATION pReloc = (PIMAGE_BASE_RELOCATION)(dwUser32Base+
		+ pUser32NTH->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC].VirtualAddress);
	...
	while ((pLoc->VirtualAddress + pLoc->SizeOfBlock) != 0) //开始扫描重定位表
	{
		...
		for ( int i=0 ; i < nNumberOfReloc; i++)
		{
			...
			else if ((pLocData[i] & 0xF000) == 0x3000) //这是一个需要修正的地址
			{
				...
				PDWORD pOldAddr = (PDWORD)((PBYTE)dwUser32Base + pReloc->VirtualAddress + (pRelocData[i] & 0x0FFF));
				*pAddress = *pOldAddr;
#endif
			}
		}
